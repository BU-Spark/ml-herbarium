...
output_dir = '/projectnb/sparkgrp/ml-herbarium-grp/ml-herbarium-angeline1/ml-herbarium/corpus/synonym/'
...
def preprocess_data():
    ...
    if not os.path.exists(output_dir + 'backbone/'):
    ...
    if not os.path.exists(output_dir + 'dic.pkl'):
        df = pd.read_csv(output_dir + txt_dir, sep='\t', low_memory=False)
...
def syn_pure(syn):
    ...
    with open('syn_pure.pkl', 'wb') as f:
