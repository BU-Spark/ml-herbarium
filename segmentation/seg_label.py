...
craft_res_dir = "CRAFT-pytorch-master/result"
boxes = []
# imgs = []
...
org_img_dir = "CRAFT-pytorch-master/in_data"
imgs = []
fnames = []
...
# expands the boxes detected by craft
def expand_boxes(boxes, diff_axes=False, mx=20, my=40, m=6):
    if not diff_axes:
        mx = m
        my = m
	...
	for i,bxs in enumerate(boxes):
		...
		for bx in bxs:
	        ...
	        if len(tmp_crop) > 0 and len(tmp_crop[0]) > 0:
	            im_lines.append(tmp_crop)
        
    	line_crops.append(im_lines)
    return line_crops
...
lines = crop_lines(lines)

# save cropped labels
save_dir = "CRAFT-pytorch-master/labels"
